...
def _setup_animation_range_from_nodes(stage, nodes):
    ...
    stage.SetMetadata(
        "comment",
        "This Layer's start/end time codes were found using the " "start/end animations of a list of Maya nodes.",
    )
...
def _setup_animation(animation, time_codes, joints, joints_transforms, topology, joint_count):
    ...
    for time_code in time_codes:
        ...
        joint_local_space_transforms = UsdSkel.ComputeJointLocalTransforms(topology, joint_world_space_transforms)
...
def _setup_cached_extents_hints(stage, root_path, meshes, times):
    ...
    for time_code in times:
        bounding_box = [Gf.Vec3f(*point) for point in mesher.get_overall_bounding_box(meshes, time_code)]
...
def _setup_rest_transforms(nodes, skeleton, time_code):
    ...
    attribute.SetMetadata("comment", "Every value in this attribute is a local-space transform.")
...
def _validate_topology(paths):
    ...
    if not valid:
        raise RuntimeError('Topology failed because of reason "{reason}".'.format(reason=reason))
...
def write_rig_as_usdskel(node, root_path, animation_path, folder, times=None):
    ...
    joint_world_space_transforms = animator.get_joint_world_space_transforms(nodes, times)
    ...
    _setup_animation_connections(main_stage, root.GetPrim().GetPath(), animation.GetPrim().GetPath())

    animation_stage.Save()

    meshes = set(mesh for joint in nodes for mesh in mesher.get_connected_meshes(joint))
    data = _setup_meshes(meshes, skeleton.GetPrim(), os.path.join(folder, "meshes.usda"))

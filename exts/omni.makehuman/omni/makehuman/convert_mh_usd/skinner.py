...
def _get_skin_cluster(meshes):
    ...
    for mesh in meshes:
        ...
        if len(clusters) > 1:
            raise NotImplementedError("This function doesn't support multiple skin clusters yet")
...
def _get_vertices(mesh):
    return [mesh + ".vtx[{}]".format(index) for index in range(cmds.polyEvaluate(mesh, vertex=True))]
...
def _calculate_influences(mesh, cluster, binding_joints, maximum_influences, weights_getter):
    indices = []
    weights = []
    ...
    for vertex in _get_vertices(mesh):
        ...
        for joint in vertex_influence_joints:
            ...
            weights.append(cmds.skinPercent(cluster, vertex, transform=joint, normalize=True, q=True))

    return indices, weights


def _write_influence_data(binding, mesh, cluster, binding_joints, maximum_influences, weights_getter):
    matrix = cmds.xform(mesh, matrix=True, worldSpace=True, q=True)
    binding.CreateGeomBindTransformAttr().Set(Gf.Matrix4d(*matrix))

    indices, weights = _calculate_influences(mesh, cluster, binding_joints, maximum_influences, weights_getter)
...
def setup_skinning(data, joints):
    ...
    if not cluster:
        raise RuntimeError('No skin cluster could be found from data "{data}".'.format(data=data))

    all_influence_joints = cmds.skinCluster(cluster, query=True, weightedInfluence=True)
    all_influence_joints = [cmds.ls(joint, long=True)[0] for joint in all_influence_joints]
    _get_weighted_joints_cached = functools.partial(_get_weighted_joints, influences=all_influence_joints)

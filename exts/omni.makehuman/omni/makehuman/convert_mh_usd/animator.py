...
def get_animation_range(nodes):
    ...
    if start == _start_default or end == _end_default:
        raise RuntimeError('No start/end time could be found from nodes, "{nodes}".'.format(nodes=sorted(nodes)))
...
def get_node_transforms_at_times(nodes, times, space):
    ...
    try:
        ...
        raise ValueError(
            'Space "{space}" is invalid. Options were, "{spaces}".'.format(space=space, spaces=sorted(spaces))
        )
    ...
    with helper.UndoChunk():
        ...
        for node in nodes:
            ...
            for time_code in times:
                time_transforms.append(Gf.Matrix4d(*cmds.getAttr(matrix_input, time=time_code)))

...
def edit_blendshapes(prim: Usd.Prim, blendshapes: Dict[str, float], time = 0):
    ...
    usd_context = omni.usd.get_context()
    stage = usd_context.get_stage()
    skeleton_rel = UsdSkel.BindingAPI(prim).GetSkeletonRel()
    skeleton_path = skeleton_rel.GetTargets()[0]
...
def bake_rig(prim : Usd.Prim, rig_path: str, time: int = 0):
    """Rig a human prim and bake the modifiers. The modifiers cannot be changed after this without re-rigging."""
    stage = prim.GetStage()
    # Get all meshes that are not joints
    non_joint_meshes = [m for m in prim.GetPrim().GetChildren() if m.IsA(UsdGeom.Mesh) and not m.GetName().startswith("joint")]
    rig = load_skel_json(os.path.join(rig_path, "default.mhskel"))
    verts = np.array(non_joint_meshes[0].GetPrim().GetAttribute("points").Get())
    skeleton = create_skeleton(stage, prim, rig, verts)

    weights_json = os.path.join(rig_path, "default_weights.mhw")
    joint_indices, weights = vertices_to_weights(skeleton.GetJointNamesAttr().Get(), weights_json, prim.GetPrim().GetChildren()[0])
    ...
    prim.SetCustomDataByKey("baked", True)
    return
